...
module.exports = function (file, api, options) {
  ...
  // Sometimes files don't have any modifications or tests,
  // ignore these
  if (typeof testsToMark !== 'undefined') {
    ...
    j(file.source).find(j.Comment).forEach(path => {
      ...
      // (if it exists)
      if (typeof path.parentPath.parentPath.value.expression !== 'undefined'){
        if (typeof path.parentPath.parentPath.value.expression.arguments !== 'undefined') {
      ...
      .filter(
        path => {
          if (typeof path.value.arguments[0] !== 'undefined'){
            if (typeof path.value.arguments[0].value !== 'undefined') {
